# IFC Lens, Solar module: clear sky irradiation evidence

This folder holds a reproducible cross check of the IFC Lens **Irradiation** metric, the optional
...
2. **The integration.** The number a user sees is a whole year accumulated. Integrating a correct
   instantaneous model incorrectly is easy and invisible: it produces a plausible figure that is
   simply wrong. **That is what this folder checks**, against a different model built by different
   people.
...
are genuinely different formulations with different aerosol treatments, so:
...
Which means the percentage is the weaker half of this evidence. **The substantive claim is the
...
is precisely how an integration error hides, so the product's suite carries the hourly total
...
the script and the test suite, both of which anyone can run; the figures are here so you can see
where in each tool the quantity lives, and they are captioned literally rather than being allowed
to imply agreement they do not show. A figure that is presented as the evidence, and then read
carefully by someone who notices it does not match the table, costs more credibility than it ever
bought.
...
The IFC Lens Irradiation metric in use: an **urban** scene at Warsaw, 2023, Average clarity, ground
albedo 0.2. The open ground points read about **1600 to 1617 kWh/m²**, a little under the
1652.7 kWh/m² in the table above, and the difference is expected rather than a discrepancy: the
comparison figure is an **unobstructed** horizontal sensor over the **full** year, while this run
...
![IFC Lens annual clear sky](figures/ifc-lens-annual-clear-sky.png)
...
Open the IFC Lens Validation report and read **Pillar 04, Clear sky irradiance**. Our column there
...
clear_sky_irradiation/
  ...
  figures/
    ...
    ifc-lens-annual-clear-sky.png      EnergyFlowX IFC Lens on the same site
  scene/
...
EnergyFlowX, the IFC Lens and the Solar tool are the property of Synerset. See the repository root
